// Package postgres provides an opt-in real PostgreSQL engine that backs
// CloudEmu's relational-database instances, so clients can run actual SQL
// against the emulator instead of a synthetic endpoint.
//
// It lives in a separate module on purpose: its heavyweight dependency (a real
...
// with config.WithDatabaseEngine(postgres.New(0)).
package postgres
...
// New returns a Postgres engine that listens on port (0 = default). The
// server starts lazily on the first Provision.
func New(port int) *Postgres {
	if port == 0 {
		port = defaultPort
	}
