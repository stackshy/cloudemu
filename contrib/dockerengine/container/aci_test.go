...
func TestACIContainerGroupE2E(t *testing.T) {
	...
	groupsClient, err := armcontainerinstance.NewContainerGroupsClient(subID, dtest.FakeCred{}, dtest.ARMOptions(ts))
	if err != nil {
		t.Fatalf("groups client: %v", err)
	}

	containersClient, err := armcontainerinstance.NewContainersClient(subID, dtest.FakeCred{}, dtest.ARMOptions(ts))
	if err != nil {
	...
	//    carries the marker.
	logs, err := containersClient.ListLogs(ctx, rg, group, containerName, &armcontainerinstance.ContainersClientListLogsOptions{
