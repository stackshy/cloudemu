...
var (
	...
	errMySQLRun      = errors.New("dockerengine: mysql container run failed")
...
// New returns a MySQL engine that publishes the container's port on the host
// (0 = default 3306). The container starts lazily on the first Provision.
func New(port int) *MySQL {
	if port == 0 {
...
func (m *MySQL) runContainer(ctx context.Context) (string, error) {
	...
	out, err := exec.CommandContext(ctx, dockerx.Binary, "run", "-d", "-p", portMap, "-e", env, mysqlImage).CombinedOutput()
	if err != nil {
		return "", fmt.Errorf("%w: %s: %w", errMySQLRun, strings.TrimSpace(string(out)), err)
	}
