...
// New returns a Compute engine. By default instances run in a small
// shell-capable base image; override it with WithBaseImage.
func New(opts ...Option) *Compute {
...
func (c *Compute) runContainer(ctx context.Context, instanceID string) (string, error) {
	name := vmNamePrefix + dockerx.SanitizeName(instanceID)

	var stdout, stderr bytes.Buffer

	//nolint:gosec // first-party argv, never a shell string
	cmd := exec.CommandContext(ctx, dockerx.Binary, "run", "-d", "--name", name, c.image, "sleep", "infinity")
...
func runBoot(ctx context.Context, containerID string, script []byte, env map[string]string) ([]byte, error) {
	...
	args = append(args, dockerx.EnvArgs(env)...)
...
func containerIP(ctx context.Context, containerID string) string {
	...
	//nolint:gosec // first-party argv, never a shell string
	out, err := exec.CommandContext(ctx, dockerx.Binary, "inspect", "-f", tmpl, containerID).Output()
	if err != nil {
		return ""
	}
