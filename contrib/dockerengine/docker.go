...
// dockerBinary is the CLI the runner shells out to.
...
func (Runner) Run(ctx context.Context, image string, cmd []string, env map[string]string, detached bool) (string, error) {
	...
	c := exec.CommandContext(ctx, dockerBinary, buildRunArgs(image, cmd, env, detached)...)
	c.Stdout = &out
	c.Stderr = &out
...
func (Runner) Inspect(ctx context.Context, id string) (state string, exitCode int, err error) {
	...
	out, err := exec.CommandContext(ctx, dockerBinary, args...).Output()
	if err != nil {
...
func (Runner) Rm(ctx context.Context, id string) error {
	if err := exec.CommandContext(ctx, dockerBinary, "rm", "-f", id).Run(); err != nil {
