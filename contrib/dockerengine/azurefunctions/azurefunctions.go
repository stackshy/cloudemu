...
// Option configures an AzureFunctions engine.
type Option func(*AzureFunctions)
...
func WithFunctionsImage(image string) Option {
...
func WithFunctionsPlatform(platform string) Option {
	return func(a *AzureFunctions) {
		a.platform = platform
	}
}

// New returns an AzureFunctions engine. Containers are created on
// Deploy and torn down on Remove/Close; the docker CLI must be on PATH (gate with
// dockerx.Available).
func New(opts ...Option) *AzureFunctions {
	a := &AzureFunctions{
...
func (a *AzureFunctions) startApp(ctx context.Context, fn config.FunctionDeployment, dir string) (funcApp, error) {
	name := funcNamePrefix + dockerx.SanitizeName(fn.Name)
...
func (a *AzureFunctions) runContainer(ctx context.Context, name, dir string, env map[string]string) (string, error) {
	...
	cmd := exec.CommandContext(ctx, dockerx.Binary, args...)
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
...
func publishedPort(ctx context.Context, id string) (int, error) {
	//nolint:gosec // first-party argv, never a shell string
	out, err := exec.CommandContext(ctx, dockerx.Binary, "port", id, funcHostContainerPort+"/tcp").Output()
	if err != nil {
