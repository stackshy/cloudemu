...
// NewContainers returns a Containers engine. Containers are created on Run and
// torn down on Stop/Close; the docker CLI must be on PATH (gate with Available).
...
func (c *Containers) Run(ctx context.Context, spec config.ContainerRunSpec) (string, error) {
	...
	for i := range spec.Containers {
		...
		dockerName := taskNamePrefix + sanitizeName(handle) + "-" + sanitizeName(cs.Name)
...
func runForeground(ctx context.Context, dockerName string, cs *config.ContainerSpec) error {
	...
	cmd := exec.CommandContext(ctx, dockerBinary, containerRunArgs(false, dockerName, cs)...)
...
func runDetached(ctx context.Context, dockerName string, cs *config.ContainerSpec) (string, error) {
	...
	//nolint:gosec // first-party argv, never a shell string
	cmd := exec.CommandContext(ctx, dockerBinary, containerRunArgs(true, dockerName, cs)...)
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
