...
func validateBatchSize(sd *streamData, entries []driver.PutRecordsRequestEntry) error {
	...
	for i := range entries {
		if len(entries[i].Data) > limit {
			return limitErr("record %d data of %d bytes exceeds the %d-byte limit",
	...
	if total > maxBatchBytes {
		return limitErr("PutRecords batch of %d bytes exceeds the %d-byte limit", total, maxBatchBytes)
	}
...
func (m *Mock) PutRecord(ctx context.Context, in driver.PutRecordInput) (*driver.PutRecordResult, error) {
	...
	if limit := recordSizeLimit(sd); len(in.Data) > limit {
		sd.mu.Unlock()
		return nil, limitErr("record data of %d bytes exceeds the %d-byte limit", len(in.Data), limit)
	}
...
func (m *Mock) PutRecords(
	...
	if len(entries) > maxBatchRecords {
		return nil, 0, limitErr("a PutRecords request may contain at most %d records, got %d",
