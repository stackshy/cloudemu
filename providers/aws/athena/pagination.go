...
// defaultPageSize is the page size Athena applies when MaxResults is unset.
...
// with an InvalidRequestException rather than silently restarting at page one.
func decodeToken(token string) (int, error) {
	...
	if err != nil {
		return 0, invalidRequest("invalid pagination token")
	}
...
// page plus the next token ("" when the last page is returned).
//
