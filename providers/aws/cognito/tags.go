...
func (m *Mock) ListTagsForResource(_ context.Context, resourceARN string) (map[string]string, error) {
	if resourceARN == "" {
		return nil, invalidParameter("ResourceArn is required")
	}

	return m.currentTags(resourceARN), nil
}

// currentTags returns a copy of the tags recorded under an ARN (nil when none).
func (m *Mock) currentTags(resourceARN string) map[string]string {
	m.tagsMu.RLock()
	defer m.tagsMu.RUnlock()

	return copyStringMap(m.tags[resourceARN])
}
...
// replaceTags replaces the entire tag set recorded under an ARN. An empty set
// clears the tags, matching UpdateUserPool's UserPoolTags full-replace behavior.
func (m *Mock) replaceTags(resourceARN string, tags map[string]string) {
	m.tagsMu.Lock()
	defer m.tagsMu.Unlock()
	...
	m.tags[resourceARN] = copyStringMap(tags)
}
