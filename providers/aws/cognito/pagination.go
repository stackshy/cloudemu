...
// defaultPageSize is the page size Cognito applies when MaxResults is unset.
...
// rather than silently restarting at page one.
func decodeToken(token string) (int, error) {
	...
	if err != nil {
		return 0, invalidParameter("invalid pagination token")
	}
...
// paginate slices items per the page's NextToken/MaxResults and returns the page
// plus the next token ("" when the last page is returned).
//
//nolint:gocritic // unnamedResult: (page, nextToken, err) is idiomatic and self-explanatory
func paginate[T any](items []T, page driver.Pagination) ([]T, string, error) {
