...
func (m *Mock) GetQueryResults(
	...
	q, ok := m.queries.Get(queryID)
	if !ok {
		return nil, errQueryNotFound(queryID)
	}

	return &driver.QueryResults{QueryStatus: q.Status, ResultRows: []map[string]string{}}, nil
}
...
func (m *Mock) CancelQuery(_ context.Context, _, queryID string) (string, error) {
	...
	q.Status = driver.QueryStatusCancelled
...
func (m *Mock) ListQueries(
	...
	for id, q := range all {
		if edsID == "" || q.EventDataStoreID == edsID {
			ids = append(ids, id)
		}
	}
