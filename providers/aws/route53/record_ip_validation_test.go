...
// AAAA value that is not an address of the right family, and store nothing.
func TestRecordRejectsBadAddress(t *testing.T) {
	ctx := context.Background()
	m := newTestMock()
	zone := createTestZone(m)
	...
	for _, cfg := range bad {
		...
		if _, err := m.GetRecord(ctx, zone.ID, cfg.Name, cfg.Type); err == nil {
			t.Fatalf("record %s stored although rejected", cfg.Name)
		}
	}

	_, err := m.UpdateRecord(ctx, driver.RecordConfig{
		ZoneID: zone.ID, Name: "www.example.com", Type: "A", TTL: 300, Values: []string{"999.1.1.1"},
	})
