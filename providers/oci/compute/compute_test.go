...
// A caller-controlled count sizes the launch's allocations, so an oversized
// one is rejected before anything is allocated or created.
func TestLaunchRejectsOversizedCount(t *testing.T) {
	// Just past the per-call maximum, and a count that would exhaust memory if
	// it ever reached an allocation.
	for _, count := range []int{1001, 1 << 40} {
		f := newFixture(t)
