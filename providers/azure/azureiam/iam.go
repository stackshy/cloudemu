...
// CreateInstanceProfile creates a new managed identity (instance profile).
func (m *Mock) CreateInstanceProfile(
	...
	if cfg.Name == "" {
		return nil, cerrors.New(
			...
			"managed identity name is required",
		)
	}
	...
	if m.instanceProfiles.Has(cfg.Name) {
		return nil, cerrors.Newf(
			cerrors.AlreadyExists,
			"managed identity %q already exists", cfg.Name,
		)
	}

	id := idgen.GenerateID("azure-mi-")
	arn := idgen.AzureID(
		m.opts.AccountID, "cloud-mock",
		"Microsoft.ManagedIdentity",
		"userAssignedIdentities", cfg.Name,
	)
...
// DeleteInstanceProfile deletes the managed identity with the given name.
func (m *Mock) DeleteInstanceProfile(
	...
	if !m.instanceProfiles.Delete(name) {
		return cerrors.Newf(
			cerrors.NotFound,
			"managed identity %q not found", name,
		)
	}
...
// GetInstanceProfile returns the managed identity with the given name.
...
// ListInstanceProfiles returns all managed identities.
...
// AddRoleToInstanceProfile associates a role with a managed identity.
func (m *Mock) AddRoleToInstanceProfile(
	...
	if !ok {
		return cerrors.Newf(
			cerrors.NotFound,
			"managed identity %q not found", profileName,
		)
	}
	...
	if p.RoleName != "" {
		return cerrors.Newf(
			cerrors.AlreadyExists,
			"managed identity %q already has role %q",
...
// RemoveRoleFromInstanceProfile removes a role from a managed identity.
func (m *Mock) RemoveRoleFromInstanceProfile(
	...
	if p.RoleName != roleName {
		return cerrors.Newf(
			cerrors.NotFound,
			"role %q is not associated with managed identity %q",
