...
func (m *Mock) childARN(server, subType, name string) string {
	return idgen.AzureID(m.opts.AccountID, resourceGroupTag, providerNamespace,
		resourceTypeFlexible+"/"+server+"/"+subType, name)
}
...
func (m *Mock) requireServer(server string) error {
	if _, ok := m.instances.Get(server); !ok {
		return cerrors.Newf(cerrors.NotFound, "MySQL Flexible Server %q not found", server)
	}
