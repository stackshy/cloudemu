...
// lbSnapshot is the full serialized state of the Cloud Load Balancing mock. The
...
type lbSnapshot struct {
	...
	GCPResources json.RawMessage                            `json:"gcpResources,omitempty"`
...
func (m *Mock) snapshotStores(snap *lbSnapshot) error {
	dumps := []struct {
		...
		{&snap.GCPResources, m.gcpResources.Snapshot},
	}

	for _, d := range dumps {
		b, err := d.fn()
		if err != nil {
			return fmt.Errorf("gcp loadbalancer: snapshot store: %w", err)
		}
...
func (m *Mock) Restore(_ context.Context, data json.RawMessage) error {
	...
	if err := json.Unmarshal(data, &snap); err != nil {
		return fmt.Errorf("gcp loadbalancer: parse snapshot: %w", err)
	}
...
func (m *Mock) restoreStores(snap *lbSnapshot) error {
	loads := []struct {
		...
		{snap.GCPResources, m.gcpResources.LoadSnapshot},
	}
	...
	for _, l := range loads {
		...
		if err := l.fn(l.src); err != nil {
			return fmt.Errorf("gcp loadbalancer: restore store: %w", err)
		}
	}
