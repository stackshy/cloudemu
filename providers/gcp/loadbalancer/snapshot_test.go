...
func seedLB(t *testing.T, m *Mock) (lbARN, tgARN string) {
	...
	require.NoError(t, m.RegisterTargets(ctx, tgARN, []driver.Target{
		{ID: "inst-aaaa", Port: 80}, {ID: "inst-bbbb", Port: 8080},
	}))
...
// TestSnapshotRestoreRoundTrip proves the Cloud Load Balancing mock serializes
// its entire state and restores it into a fresh mock identity-preservingly.
