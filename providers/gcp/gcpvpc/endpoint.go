...
func (m *Mock) CreateVPCEndpoint(
	...
	if cfg.VPCID == "" {
		return nil, cerrors.Newf(
			cerrors.InvalidArgument, "VPC ID is required",
		)
	}
	...
	if !m.vpcs.Has(cfg.VPCID) {
		return nil, cerrors.Newf(
			cerrors.NotFound, "VPC %q not found", cfg.VPCID,
		)
	}
...
// DeleteVPCEndpoint deletes the VPC service endpoint with
// the given ID.
...
// DescribeVPCEndpoints returns VPC service endpoints matching
// the given IDs, or all endpoints if ids is empty.
