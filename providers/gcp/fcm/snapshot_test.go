...
// TestSnapshotRestoreRoundTrip proves the FCM mock serializes its entire state
// and restores it into a fresh mock identity-preservingly.
func TestSnapshotRestoreRoundTrip(t *testing.T) {
	ctx := context.Background()

	src := newTestMock()

	_, err := src.CreateTopic(ctx, driver.TopicConfig{Name: "news"})
	require.NoError(t, err)

	_, err = src.Subscribe(ctx, driver.SubscriptionConfig{
		TopicID: "news", Protocol: "https", Endpoint: "https://example.com/hook",
	})
	require.NoError(t, err)

	_, err = src.Publish(ctx, driver.PublishInput{TopicID: "news", Message: "ping"})
	require.NoError(t, err)
	...
	topic, err := dst.GetTopic(ctx, "news")
	require.NoError(t, err)
	require.NotNil(t, topic)

	subs, err := dst.ListSubscriptions(ctx, "news")
	require.NoError(t, err)
	assert.Len(t, subs, 1)
}
