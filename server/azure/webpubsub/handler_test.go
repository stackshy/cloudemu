...
const (
	...
	basePath = "/subscriptions/sub1/resourceGroups/rg1/providers/Microsoft.SignalRService/webPubSub/"
)
...
func newServer(t *testing.T) *httptest.Server {
	...
	mock := webpubsub.New(config.NewOptions())
	h := webpubsubsrv.New(mock)
...
func TestWireCreateAndComputedFields(t *testing.T) {
	...
	got := decode(t, raw)
	if got.Type != "Microsoft.SignalRService/webPubSub" {
		t.Errorf("type = %q", got.Type)
	}

	if got.Properties.HostName != "wps1.webpubsub.azure.com" {
	...
	if got.Kind != "WebPubSub" {
		t.Errorf("kind = %q, want WebPubSub", got.Kind)
	}

	if got.Properties.LiveTrace == nil || got.Properties.LiveTrace.Enabled != "true" ||
		len(got.Properties.LiveTrace.Categories) != 1 {
		t.Errorf("liveTrace = %+v", got.Properties.LiveTrace)
	}
}
...
func TestWirePatchMergesAndPreservesComputed(t *testing.T) {
	...
	// liveTrace was not mentioned in the PATCH, so it must survive.
	if after.Properties.LiveTrace == nil || after.Properties.LiveTrace.Enabled != "true" {
		t.Errorf("liveTrace not preserved on patch: %+v", after.Properties.LiveTrace)
	}
}
...
func TestWireListKeysIsStable(t *testing.T) {
	...
	keysPath := basePath + "wps1/listKeys" + apiVer
