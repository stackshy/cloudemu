// Package webpubsub serves the Azure Web PubSub Service ARM API
// (Microsoft.SignalRService/webPubSub). Real armwebpubsub WebPubSubClient
// requests hit this handler the same way they hit management.azure.com.
//
// Web PubSub is the sibling of Azure SignalR Service under the same
// Microsoft.SignalRService provider, so this handler mirrors the signalR handler
// almost exactly. Every operation is synchronous (sync-200/201):
// CreateOrUpdate, Delete and the listKeys action complete in-line, so there is
// no long-running-operation plumbing to wire.
package webpubsub
...
// Store is the minimal webPubSub backend the handler needs. *webpubsub.Mock
// satisfies it.
type Store interface {
	CreateOrUpdate(ctx context.Context, sub, rg, name string, in *webpubsub.Input) (webpubsub.WebPubSub, bool, error)
	Get(ctx context.Context, sub, rg, name string) (webpubsub.WebPubSub, error)
	Delete(ctx context.Context, sub, rg, name string) (bool, error)
	ListByResourceGroup(ctx context.Context, sub, rg string) ([]webpubsub.WebPubSub, error)
	ListBySubscription(ctx context.Context, sub string) ([]webpubsub.WebPubSub, error)
	PurgeResourceGroup(ctx context.Context, sub, rg string) error
}

// Handler serves Microsoft.SignalRService/webPubSub ARM requests.
type Handler struct {
	store Store
}

// New returns a webPubSub handler backed by store.
func New(store Store) *Handler {
	return &Handler{store: store}
}

// Matches reports whether r targets a webPubSub ARM URL. The provider and type
...
// serveAction routes the POST sub-resource actions on a named webPubSub resource.
...
func (h *Handler) delete(w http.ResponseWriter, r *http.Request, rp *azurearm.ResourcePath) {
	...
	// deleted one returns 200 OK. The armwebpubsub client accepts both.
	if existed {
...
func (h *Handler) list(w http.ResponseWriter, r *http.Request, rp *azurearm.ResourcePath) {
	...
	out := listResponse{Value: make([]webPubSubResponse, 0, len(items))}
	for i := range items {
...
func inputFromRequest(req *webPubSubRequest) webpubsub.Input {
	in := webpubsub.Input{
...
func applyProperties(in *webpubsub.Input, p *propertiesRequest) {
	if p == nil {
		return
	}
	...
	in.LiveTrace = toDriverLiveTrace(p.LiveTrace)
...
func mergePatch(existing *webpubsub.WebPubSub, req *webPubSubRequest) webpubsub.Input {
...
func mergeProperties(in *webpubsub.Input, p *propertiesRequest) {
	if p == nil {
		return
	}
...
func inputFromExisting(s *webpubsub.WebPubSub) webpubsub.Input {
...
func identityInputFrom(in *webpubsub.Identity) *webpubsub.Identity {
	if in == nil {
		return nil
	}
