...
func toAzureNSGRules(in []securityRule) []netdriver.AzureNSGRule {
	...
	for i := range in {
		...
		out = append(out, netdriver.AzureNSGRule{
			...
			SourceASGs:               refIDs(p.SourceApplicationSecurityGroups),
			DestinationASGs:          refIDs(p.DestinationApplicationSecurityGroups),
			Access:                   p.Access,
	...
	for i := range in {
		...
		out = append(out, securityRule{
			...
			Properties: securityRuleProps{
				...
				SourceApplicationSecurityGroups:      toIDRefs(r.SourceASGs),
				DestinationApplicationSecurityGroups: toIDRefs(r.DestinationASGs),
				Access:                               r.Access,
				Priority:                             r.Priority,
