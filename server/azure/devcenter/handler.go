// Package devcenter serves the Azure Dev Center ARM API
// (Microsoft.DevCenter/devcenters). Real armdevcenter DevCentersClient requests
...
const (
	providerName = "Microsoft.DevCenter"
	resourceType = "devcenters"
	armType      = providerName + "/" + resourceType
)

// Store is the minimal dev center backend the handler needs.
// *devcenter.Mock satisfies it.
type Store interface {
	CreateOrUpdate(
		ctx context.Context, sub, rg, name, location string, in *devcenter.Input,
	) (devcenter.DevCenter, bool, error)
	Get(ctx context.Context, sub, rg, name string) (devcenter.DevCenter, error)
	Delete(ctx context.Context, sub, rg, name string) (bool, error)
	ListByResourceGroup(ctx context.Context, sub, rg string) ([]devcenter.DevCenter, error)
	ListBySubscription(ctx context.Context, sub string) ([]devcenter.DevCenter, error)
	PurgeResourceGroup(ctx context.Context, sub, rg string) error
}

// Handler serves Microsoft.DevCenter/devcenters ARM requests.
type Handler struct {
	store Store
}

// New returns a dev center handler backed by store.
func New(store Store) *Handler {
	return &Handler{store: store}
}

// Matches reports whether r targets a dev center ARM URL. The provider and type
...
func (h *Handler) delete(w http.ResponseWriter, r *http.Request, rp *azurearm.ResourcePath) {
	...
	// deleted one returns 200 OK. The armdevcenter client accepts both.
	if existed {
...
func (h *Handler) list(w http.ResponseWriter, r *http.Request, rp *azurearm.ResourcePath) {
	...
	out := listResponse{Value: make([]devCenterResponse, 0, len(items))}
	for i := range items {
...
func inputFromRequest(req *devCenterRequest) devcenter.Input {
	...
	if req.Properties != nil {
		in.DisplayName = req.Properties.DisplayName

		if req.Properties.ProjectCatalogSettings != nil {
			in.CatalogItemSyncEnableStatus = req.Properties.ProjectCatalogSettings.CatalogItemSyncEnableStatus
		}
	}
