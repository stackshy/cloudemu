// Package elasticsan serves the Azure Elastic SAN ARM API
// (Microsoft.ElasticSan/elasticSans). Real armelasticsan ElasticSansClient
...
const (
	providerName = "Microsoft.ElasticSan"
	resourceType = "elasticSans"
	armType      = providerName + "/" + resourceType
)

// Store is the minimal Elastic SAN backend the handler needs.
// *elasticsan.Mock satisfies it.
type Store interface {
	CreateOrUpdate(
		ctx context.Context, sub, rg, name, location string, in *elasticsan.Input,
	) (elasticsan.ElasticSan, bool, error)
	Get(ctx context.Context, sub, rg, name string) (elasticsan.ElasticSan, error)
	Delete(ctx context.Context, sub, rg, name string) (bool, error)
	ListByResourceGroup(ctx context.Context, sub, rg string) ([]elasticsan.ElasticSan, error)
	ListBySubscription(ctx context.Context, sub string) ([]elasticsan.ElasticSan, error)
	PurgeResourceGroup(ctx context.Context, sub, rg string) error
}

// Handler serves Microsoft.ElasticSan/elasticSans ARM requests.
type Handler struct {
	store Store
}

// New returns an Elastic SAN handler backed by store.
func New(store Store) *Handler {
	return &Handler{store: store}
}

// Matches reports whether r targets an Elastic SAN ARM URL. The provider and type
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// The top-level elasticSans resource exposes no POST sub-resource actions;
	// nested volumeGroups/volumes/privateEndpointConnections are separate types.
	if rp.SubResource != "" {
...
// The total_* fields recompute when base/extended size changes. A PATCH on a
// missing resource is a 404.
...
func (h *Handler) delete(w http.ResponseWriter, r *http.Request, rp *azurearm.ResourcePath) {
	...
	// deleted one returns 200 OK. The armelasticsan client accepts both.
	if existed {
...
func (h *Handler) list(w http.ResponseWriter, r *http.Request, rp *azurearm.ResourcePath) {
	...
	out := listResponse{Value: make([]sanResponse, 0, len(items))}
	for i := range items {
