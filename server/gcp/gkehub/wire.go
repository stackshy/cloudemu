...
// and re-injected from the stored resource on every read. The union covers all
// three collections: membership (uniqueId, state), feature (resourceState,
// state, membershipStates, scopeStates, unreachable), and fleet (uid, state).
//
//nolint:gochecknoglobals // immutable lookup set
var outputKeys = map[string]bool{
	"name": true, "uid": true, "uniqueId": true, "state": true,
	"resourceState": true, "membershipStates": true, "scopeStates": true,
	"unreachable": true, "createTime": true, "updateTime": true,
	"deleteTime": true, "lastConnectionTime": true,
}
...
// toResourceJSON renders a driver resource as gkehub/v1 wire JSON, merging the
// verbatim body fields with the computed output-only fields. col.seed injects
// the per-collection computed identity/state block.
func (c *collection) toResourceJSON(r *gdriver.Resource) (json.RawMessage, error) {
...
func putScalars(m map[string]json.RawMessage, r *gdriver.Resource) error {
	if err := putJSON(m, "createTime", formatTime(r.CreateTime)); err != nil {
		return err
	}

	return putJSON(m, "updateTime", formatTime(r.UpdateTime))
}
