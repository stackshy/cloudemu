...
// toResourceJSON renders a driver resource as certificatemanager/v1 wire JSON,
// merging the verbatim body fields with the computed name/createTime/updateTime
// output fields. A collection's stripOutput hook drops write-only body values
// (a self-managed certificate's pemPrivateKey) from the rendered copy without
// touching the stored resource.
func (c *collection) toResourceJSON(r *cmdriver.Resource) (json.RawMessage, error) {
	m := make(map[string]json.RawMessage, len(r.Fields)+minComputedFields)
	for k, v := range r.Fields {
		m[k] = v
	}

	if c.stripOutput != nil {
		c.stripOutput(m)
	}
