...
func TestRestoreStateIgnoresUnreadableFile(t *testing.T) {
	...
	if err := restoreState(context.Background(), corrupt, map[string]persist.Services{}, nil, nil); err != nil {
		t.Fatalf("restoreState(corrupt) = %v, want nil (start empty)", err)
	}

	missing := filepath.Join(dir, "does-not-exist.json")
	if err := restoreState(context.Background(), missing, map[string]persist.Services{}, nil, nil); err != nil {
