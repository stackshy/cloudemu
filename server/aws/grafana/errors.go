...
// statusForException returns the HTTP status for a Grafana exception name.
...
// exceptionForCode maps a canonical cloudemu code to a Grafana exception name
...
func exceptionForCode(err error) (status int, errType string) {
	...
	case cerrors.IsPermissionDenied(err):
		return http.StatusForbidden, driver.ExAccessDenied
	default:
		return http.StatusInternalServerError, driver.ExInternalServer
	}
}

// writeErr maps a driver error to the precise Grafana exception. Tagged
...
// an update with no fields) and leaves v at its zero value.
...
// writeJSON writes a restJson1 success body with the given status code.
