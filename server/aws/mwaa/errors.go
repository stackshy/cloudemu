...
// statusForException returns the HTTP status for an MWAA exception name.
...
// exceptionForCode maps a canonical cloudemu code to an MWAA exception name for
...
// writeErr maps a driver error to the precise MWAA exception. Tagged
...
// yields an empty map (e.g. an update with no fields).
