...
// contains slashes), so rest is rejoined into the full ARN.
//
...
func (h *Handler) tagResource(w http.ResponseWriter, r *http.Request, resourceArn string) {
	...
	if err := h.aps.TagResource(r.Context(), resourceArn, tagsFromBody(raw)); err != nil {
		writeErr(w, err)
...
func (h *Handler) untagResource(w http.ResponseWriter, r *http.Request, resourceArn string) {
	...
	if err := h.aps.UntagResource(r.Context(), resourceArn, tagKeys); err != nil {
		writeErr(w, err)
...
func (h *Handler) listTagsForResource(w http.ResponseWriter, r *http.Request, resourceArn string) {
	tags, err := h.aps.ListTagsForResource(r.Context(), resourceArn)
	if err != nil {
		writeErr(w, err)
